...
$$
\begin{alignedat}{3}
P(A \mid D) &= {P(D \mid A) \cdot P(A) \over P(D)} &&= {0.9 * 0.7 \over 0.66} &&\approx 0.955 \\
...
P(\lnot A \mid \lnot D) &= {P(\lnot D \mid \lnot A) \cdot P(\lnot A) \over P(\lnot D)} &&= {0.9 * 0.3 \over 0.34} &&\approx 0.794
\end{alignedat}
$$
